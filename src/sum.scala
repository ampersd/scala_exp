...
object Program {
  ...
  def SumTail (list: List[Int], acc: Int) : Int = {
    list match {
      case Nil => acc
  ...
  def main(args: Array[String]) {
    ...
    println("Хвостовая рекурсия: " + SumTail(list, 0)) // для хвостовой рекурсии необходимо задать начальное значение аккумулятора
